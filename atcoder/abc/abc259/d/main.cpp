...
const long double EPS = 1e-14;    const ld PI = 3.14159265358979323846264338327950288;
...
// clang-format on
#pragma endregion
#pragma region datastructure / unionfind / unionfind.hpp
/**
 * @brief UnionFind
...
bool intersect(int x1, int y1, int r1, int x2, int y2, int r2) {
    long double d = distance(x1, y1, x2, y2);
    if (sign(d - r1 - r2) > 0 or sign(d + r1 - r2) < 0) return false;
    return true;
}

int solve() {

    SS(int, N, sx, sy, tx, ty);
    ...
    REP (i, N) {
        ...
        if (s_i == -1 && abs(r - distance(sx, sy, x, y)) < EPS) {
            s_i = i;
        }
        if (t_i == -1 && abs(r - distance(tx, ty, x, y)) < EPS) {
            t_i = i;
        }
    }
