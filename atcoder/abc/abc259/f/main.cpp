...
// dp[頂点][そこに到達したときに辺を選んでdを消費したか]
ll dp[300300][2];
bool vis[300300][2];

ll rec(int cur, int prv, bool used) {
    if (vis[cur][used]) {
        return dp[cur][used];
    }
    ...
    for (auto &[nxt, cst] : graph[cur]) {
        if (nxt == prv) continue;
        ll use = (D[nxt] == 0 ? 0 : cst) + rec(nxt, cur, true);
        ll nouse = rec(nxt, cur, false);
        v.emplace_back(use - nouse, use, nouse);
    }
